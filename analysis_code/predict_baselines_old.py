...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use log(sum(decay)) for BL scores where possible. Ranking among seen items is "
            "unchanged, but BL-kNN weights are usually safer without this flag because log "
            "scores can be negative depending on timestamp scale."
        ),
    )
    parser.add_argument(
        "--csv-chunk-size",
...
def bl_item_scores(
    ...
    """
    Non-negative BL strength per item by default: sum_j (t - tj + c)^(-d).
    With use_log=True, seen item scores become log(strength); unseen items remain -inf.
    """
...
def top_items_from_scores(
    ...
    For default non-log BL scores, unseen items have score 0 and seen items have
    positive scores. For --use-log-bl, unseen items are -inf and seen items can be
    negative, so we rank all finite items.
    """
