...
CURVES_PKL = Path("./curves_all_models_synthetic.pkl")
...
# def setup_plot_style(style_module) -> None:
#     """Apply thesis style, but do not use the PGF backend for PDF export."""
#     style_module.setup_mpl(use_latex_pgf=True, grid=True)

def setup_plot_style(style_module) -> None:
    """Apply thesis style, but do not let constrained layout fight manual spacing."""
    style_module.setup_mpl(use_latex_pgf=True, grid=True)
...
def make_track_df(start_ts, end_ts, n_points, track_id, user_id):
    """
    Create a dataframe with constant track_id/user_id and evenly spaced Unix timestamps.
    ...
    n_points : int
        Number of timestamp points to generate.
    track_id : any
    user_id : any
    """
    ts = np.linspace(start_ts, end_ts, n_points, dtype=int)
...
def load_similarity_row_for_learned_model(df_full: pd.DataFrame, item_id: int) -> np.ndarray:
    ...
    config = {'n_users': 13209, 'n_items': 3027, 'latent_d': 64,
    ...
    state_dict = {
        key.replace("metric._orig_mod.", "metric."): value
...
def plot_dual_interaction_impact(
    ...
    # fig = plt.figure(figsize=(6.3, 6.6), facecolor="white")

    fig = plt.figure(figsize=(6.3, 6.6), facecolor="white", constrained_layout=False)
    try:
    ...
    ax_title.text(
        0.5,
        0.68,
        "Interaction relevance vs. probability and score over time",
        ha="center",
        va="center",
    ...
    # Explicit margins so title / labels / colorbar do not collide.
    # fig.subplots_adjust(left=0.16, right=0.88, top=0.90, bottom=0.10)
    fig.subplots_adjust(left=0.16, right=0.88, top=0.97, bottom=0.10)
    return fig
...
def main() -> None:
    ...
    stem = f"interaction_curves_{args.model}_user_{args.user}_item_{args.item}_synthetic"
