...
# Hardcoded heatmap styling. Overwrite these values in the script if desired.
HEATMAP_CMAP = "rainbow"
HEATMAP_LOG_VMIN = 1e-4
HEATMAP_LOG_VMAX = 1.0
...
def setup_thesis_plot_style():
    """Apply the shared thesis Matplotlib style to all plots in this script."""
    setup_mpl(use_latex_pgf=True, grid=True)

    # This script uses explicit fig.tight_layout() calls. Disable constrained
    # layout from the shared style so the two layout engines do not conflict.
    import matplotlib as mpl
    mpl.rcParams["figure.constrained_layout.use"] = False


def save_thesis_figure(fig, output_path):
    """Save plot in the original format plus thesis-friendly PDF and PGF."""
    output_path = Path(output_path)
    original_fmt = output_path.suffix.lstrip(".") or "png"

    # Preserve the script's original output file, and additionally emit PDF/PGF
    # files with the same stem for LaTeX inclusion.
    formats = []
    for fmt in (original_fmt, "pdf", "pgf"):
...
def plot_clustered_heatmap(S, Z, labels, item_indices, output_path):
    with timed_step("Plotting clustered similarity heatmap"):
        ...
        # LogNorm cannot display zeros. Clip only for visualization; the
        # underlying similarity matrix used for clustering/statistics is unchanged.
        S_for_plot = np.clip(S_ordered, HEATMAP_LOG_VMIN, HEATMAP_LOG_VMAX)

        fig, ax = plt.subplots(figsize=fig_size(fraction=1.0, height_in=4.8), constrained_layout=False)
        image = ax.imshow(
            S_for_plot,
            aspect="auto",
            interpolation="nearest",
            cmap=HEATMAP_CMAP,
            norm=LogNorm(vmin=HEATMAP_LOG_VMIN, vmax=HEATMAP_LOG_VMAX),
        )
        fig.colorbar(image, ax=ax, label=r"Similarity (log scale, $10^{-4}$ to $1$)")

        ax.set_title("Clustered relevance matrix.")
...
def plot_dendrogram(Z, n, candidate, output_path):
    with timed_step("Plotting truncated dendrogram"):
        fig, ax = plt.subplots(figsize=fig_size(fraction=1.0, height_in=3.0), constrained_layout=False)
...
def plot_mds(coords, labels, item_indices, output_path, show_labels=False):
    with timed_step("Plotting 2D Classical MDS / PCoA"):
        fig, ax = plt.subplots(figsize=fig_size(fraction=1.0, height_in=4.0), constrained_layout=False)
...
def plot_scan_diagnostics(candidates, output_dir):
    with timed_step("Plotting scan diagnostics"):
        ...
        if len(k_df) > 0:
            ...
            fig.tight_layout()
            save_thesis_figure(fig, output_dir / "diagnostic_k_largest_cluster_fraction.png")
            ...
            fig.tight_layout()
            save_thesis_figure(fig, output_dir / "diagnostic_k_separation.png")
            ...
            fig.tight_layout()
            save_thesis_figure(fig, output_dir / "diagnostic_k_silhouette.png")
        ...
        if len(t_df) > 0:
            ...
            fig.tight_layout()
            save_thesis_figure(fig, output_dir / "diagnostic_threshold_n_clusters.png")
            ...
            fig.tight_layout()
            save_thesis_figure(fig, output_dir / "diagnostic_threshold_largest_cluster_fraction.png")
            ...
            fig.tight_layout()
            save_thesis_figure(fig, output_dir / "diagnostic_threshold_separation.png")
            ...
            fig.tight_layout()
            save_thesis_figure(fig, output_dir / "diagnostic_threshold_silhouette.png")
        ...
        fig.tight_layout()
        save_thesis_figure(fig, output_dir / "diagnostic_all_candidates_auto_score.png")
