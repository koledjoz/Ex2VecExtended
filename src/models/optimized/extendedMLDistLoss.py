...
class FastSymmetricPairwiseMLP(nn.Module):
    ...
    def _raw_diag_scores(self, emb: torch.Tensor) -> torch.Tensor:   # MODIFIED
        """
        Compute raw self-scores before diagonal zeroing.
        Dropout is DISABLED for stability.
        """
        zeros = torch.zeros_like(emb)
        prod = emb * emb
        ...
        h = F.linear(zeros, W_abs) + F.linear(prod, W_prod, b)  # (N, H)
        h = self.act(h)

	# MODIFIED: explicitly skip dropout (even in training)
	# if self.dropout is not None:
	#     h = self.dropout(h)
    ...
    def identity_penalty(                         # MODIFIED
        ...
        """
        Penalty on raw self-scores before forward() zeroes the diagonal.
        """
        raw_diag = self._raw_diag_scores(emb)
        if kind == "l2":
            penalty = raw_diag.pow(2)
        elif kind == "l1":
            penalty = raw_diag.abs()
        else:
    ...
    def forward(self, emb: torch.Tensor) -> torch.Tensor:
        ...
        out.fill_diagonal_(0.0)   # MODIFIED: enforce identity only at output level
        return out
